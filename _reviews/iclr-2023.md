...
* Transition : $\hat{z}_{t+1} \sim p_G(\hat{z}_{t+1} | z_{\le t}, a_{\le t})$ with  $\hat{z}_{t+1}^k \sim p_G(\hat{z}_{t+1}^k | z_{\le t}, a_{\le t}, z_{t+1}^{< k})$
* Reward: $\hat{r}_t \sim p_G(\hat{r}_t | z_{\le t}, a_{\le t})$
* Termination : $\hat{d}_t \sim p_G(\hat{d}_t | z_{\le t}, a_{\le t})$
